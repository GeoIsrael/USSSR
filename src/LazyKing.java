...
class UnluckyVassal {
    public void printReportForKing(List<String> pollResults) {
        ...
        pollResults.stream().forEach(s -> {
            if(!s.contains(":")) {
                ...
                if(!peoples[1].contains(",")) {
                    System.out.println("\t" + "\t" + peoples[1]) ;
                } else {
                    printStaff(peoples[1].split(","));
                }
    ...
    private void printStaff(String[] split) {
        Arrays.stream(split).sequential().forEach( staff -> {
